...
function buildFileName(fileName, lastNumber = 1, extension, formatDate) {
  let name = fileName;
  if (!name) {
  ...
  if (typeof fileName === "string" && fileName.includes("$DATE")) {
    const today = new Date();
    const timeStr = formatDate?.(today) ?? today.toISOString();
    name = fileName.replace("$DATE", timeStr);
  }
  return `${name}.${lastNumber}${extension ?? ""}`;
}
